...
function orbit_length(  X0::Vector{T};          # initial condition
    ...
    tic = time()
    X0 = L63(T,xyz=X0,N=lspinup,output=false)
    toc = time()
    if verbose
    ...
    while l == 0 && nchunk < nchunkmax
        ...
        X = L63(T,xyz=X0,N=lchunk,output=true)
...
Lorenz63 and test for their uniqueness."""
function find_orbits(   ::Type{T},                  # Number format
    ...
    spinup = 100
    ini = L63(Float64,N=lini+spinup,xyz=randn(3),output=true)
